...
def scan_URL(url):
    command = "python3 ./resources/VxAPI/vxapi.py scan_url_for_analysis {} all".format(
        url
    )
...
def check_Status(jsonSHA):
    command = "python3 ./resources/VxAPI/vxapi.py report_get_summary {}".format(jsonSHA)
    rawOutput = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE).stdout
...
def check_Overview(jsonSHA):
    command = "python3 ./resources/VxAPI/vxapi.py overview_get {}".format(jsonSHA)
    rawOutput = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE).stdout
...
def check_State(jsonSHA):
    command = "python3 ./resources/VxAPI/vxapi.py report_get_state {}".format(jsonSHA)
    rawOutput = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE).stdout
